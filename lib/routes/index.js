...
router.get('/', function (req, res) {
  Activity.findAll({
    where: {
      startTime: {
        between: [
          moment.utc('2015-01-01').toString(),
          moment.utc('2016-01-01').toString()
        ]
      }
    }
  })
  .then(function (activities) {
    ...
    var commuteActivities = _.filter(activities, function (activity) {
      var dayOfWeek = moment(activity.startTime).day();
      return activity.isCommute === true
        && dayOfWeek > 0
        && dayOfWeek < 6;
    });
